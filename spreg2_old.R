...
spreg2<-function(formula, data=list(), listw,
                 listw2 = NULL, endog = NULL, instruments= NULL,
  ...
  if(model %in% c("sarar","lag", "ivhac")){
    if (!is.null(endog)) {
      ...
      Emat <- cbind(endog, as.matrix(wy))
      colnames(Emat) <- c(colnames(endog), colnames(wy))
    }
    else {
  ...
  if(model %in% c("sarar","error")){
    ...
    firststep <- spatial.ivreg2(y = y , Emat = Emat, Xmat = Xmat, Hmat = Hmat, het = het, HAC = HAC, type = type, bandwidth = bandwidth, distance = distance)
    ...
    wEmat <- Ws2 %*% Emat
    Et <- Emat - rhotilde * wEmat
    wXmat <- Ws2 %*% Xmat
    Xt <- Xmat - rhotilde * wXmat
    ...
    secondstep<-spatial.ivreg2(y =yt , Emat = Et, Xmat = Xt, Hmat = Hmat, het = het, HAC=HAC, type=type, bandwidth=bandwidth, distance=distance)
...
#Replace Emat with a matrix for exogenous and endogenous variables...
spatial.ivreg2<-function(y, Emat, Xmat, Hmat,
                         het = het,
                         HAC = HAC,
                         distance = distance,
                         type = c("Epanechnikov", "Triangular", "Bisquare",
                         ...
                         bandwidth = bandwidth){
  # 2SLS estimator:
  Zmat         <- cbind(Xmat, Emat)
  df           <- nrow(Zmat) - ncol(Zmat)
  HH           <- crossprod(Hmat)
  HE           <- crossprod(Hmat, Emat)
  bz           <- solve(HH, HE)
  Ematp        <- Hmat %*% bz
  Zp           <- cbind(Xmat, Ematp)
  ZpZp         <- crossprod(Zp)
  ...
  #names(delta) <- c(colnames(Xmat), colnames(Emat))
  yp           <- Zmat %*% delta
  e            <- y - yp
  ...
  if(HAC){
    ...
    if(is.null(attributes(distance)$GeoDa$dist)){
      ...
      Ker  <- lapply(attributes(distance)$GeoDa$dist, ker.fun,
                     bandwidth = bandwidth)
...
environment(spreg2)         <- asNamespace("sphet")
environment(spatial.ivreg2) <- asNamespace("sphet")