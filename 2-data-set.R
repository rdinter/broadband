...
setvars <- function(y1.1, y1.2, y1.l, y2.1, y2.2, y3.1, y3.2 = NULL,
                    scal = T, data, W, diffy1 = F){
  est  <- data.frame(data$FIPS)
  
  #Population
  if (diffy1) est$y1   = data[, y1.2] - data[, y1.1]
  else est$y1   = data[, y1.2] + data[, y1.1]
  ...
  if (!is.null(y3.2)){
    inst <- c(vars[-1], "y1_l", "y2_l", "y3_l")
  } else inst <- c(vars[-1], "y1_l", "y2_l")
  H      <- as.matrix(est[, inst])
  WH     <- W %*% H
  colnames(WH) <- NULL
  #   WWH    <- W %*% WH
  Ph     <- cbind(1, H, WH)#, WWH)
...
setvars_log <- function(y1.1, y1.2, y1.l, y2.1, y2.2, y3.1, y3.2 = NULL,
                    scal = T, data, W, diffy1 = F){
  est  <- data.frame(data$FIPS)
  ...
  est$y1 = log((data[, y1.l] + est$y1) / data[, y1.l])
  if (scal)  est$y1   = scale(est$y1)
  ...
  est$y1_l = log(data[, y1.l])
  if (scal) est$y1_l = scale(est$y1_l)
  # est$y1_l = est$y1_l + W %*% est$y1_l
  est$Wy1_l = W %*% est$y1_l
  
  #Employment
  est$y2   = log((1 + data[, y2.2]) / data[, y2.1])
  if (scal) est$y2   = scale(est$y2)
  ...
  est$y2_l = log(data[, y2.1])
  if (scal) est$y2_l = scale(est$y2_l)
  # est$y2_l = est$y2_l + W %*% est$y2_l
  est$Wy2_l= W %*% est$y2_l
  
  #Broadband
  est$dBB   = log(data$total_prov.2010B / data$total_prov.2008B)
  est$BB    = log(data[, y3.1])
  
  if (!is.null(y3.2)){
    est$y3    = log(data[, y3.2] / data[, y3.1])
    if (scal) est$y3 = scale(est$y3)
    est$WY3   = W %*% est$y3
