spreg2<-function(formula, data=list(), listw, listw2=NULL, endog = NULL, instruments= NULL, lag.instr = FALSE, initial.value=0.2, model = c("sarar", "lag", "error", "ivhac", "ols"), het = FALSE, verbose=FALSE, na.action = na.fail,  HAC = FALSE, distance = NULL, type=c("Epanechnikov","Triangular","Bisquare","Parzen", "QS","TH"), bandwidth="variable" ,step1.c = FALSE, control = list()){
  ...
  if(model %in% c("sarar","error")){
    ...
    utildebt <- yt - Zt %*% delta #different residuals
    # print(delta)
    ...
    else  results<-list(coefficients=coeff,var=vcmat$Omega, s2=s2, call=cl, residuals=as.numeric(utildebt), model=model.data,method=method,W=W, firststep=firststep$coefficients, init.rho = rhotilde)
